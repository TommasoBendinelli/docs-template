...
\agentProposedEdit
  ...
  {Standardize every documentation repository around one explicit TeX build closure so synchronization excludes raw data and unrelated project material, as prepared in Codex conversation hash 01a03d38-a300-7be1-ac03-4b540a4f117d.}
  {}
  {Every \path{docs-*} repository uses its existing \path{tex/} tree as the complete project-local compilation boundary.
The authoritative standalone order is stored in \path{tex/docs_order_manifest.txt}, and every listed entry is an authored UTF-8 TeX file under \path{tex/source/}.
Optional authored included TeX belongs under \path{tex/fragments/}, and optional authored styles and bibliographies belong under \path{tex/support/}.
Optional authored binary compilation inputs belong under \path{tex/assets/}, and only machine-produced inputs consumed by the current canonical PDF belong under \path{tex/generated/}.
Generated TeX is a content-addressed build object under \path{tex/generated/}, rather than an editable source-frontier file.
Generator programs belong under \path{scripts/}, compact tracked manifests and provenance belong under \path{scripts/provenance/}, and ignored raw or bulky generation material belongs under \path{.cache/generation/}.
Review state remains under \path{comments/}, project material that must never be discovered implicitly remains under \path{third_party/}, retired material remains under \path{archive/}, and release packages remain under \path{submission/}.
Compilation output belongs under the ignored \path{build/} and \path{pdf/} trees.
The workspace-global \path{third_party/} directory is never enumerated, while a source-level \code{\PdfLink} may select one explicit direct PDF from that registry.
No canonical compile may read scripts, caches, archives, submissions, another project, or an undeclared global dependency.
...
A Fedora-verified dependency closure must identify every declared build object that was consumed and every project-local compiler input that was authorized.}
